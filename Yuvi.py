...
import numpy as np
def yuvi_title():
 return st.title("Yuvraj Singh T20 International Career Analysis")
...
@st.cache_data
def yuvi_performance():
    t20i = load_data()
    full_name = ''
    for name in t20i['Batter'].unique():
        if 'yuvraj' in name.lower():
            full_name = name
            break
    st.write(full_name)

    yuvraj_t20i = t20i[t20i['Batter'] == full_name]
    ...
    yuvraj_t20i = yuvraj_t20i[recommended_columns]
    st.dataframe(yuvraj_t20i.sample(5))
    st.write(yuvraj_t20i.shape)

    match_id_mapping = {}
    match_id_counter = 1

    for match_id in yuvraj_t20i['Match ID'].unique():
        match_id_mapping[match_id] = f"Match {match_id_counter}"
        match_id_counter += 1

    yuvraj_t20i['Match ID'] = yuvraj_t20i['Match ID'].map(match_id_mapping)

    matches_played = yuvraj_t20i['Match ID'].nunique()
    total_runs_scored = yuvraj_t20i['Batter Runs'].sum()
    ...
    runs_by_year = yuvraj_t20i.pivot_table(index='year', values='Batter Runs', aggfunc='sum').reset_index()
    runs_by_year['Percentage Change'] = np.round(runs_by_year['Batter Runs'].pct_change() * 100)

    st.title("Yuvraj's Runs Year by Year")
    plt.figure(figsize=(9, 4))
    sns.set_style("darkgrid")
    ...
    venue_stats = yuvraj_t20i.groupby('Venue').agg({
        'Batter Runs': 'sum',
    ...
    st.title("Yuvraj's Top 5 Favourite Stadiums to Score")
    plt.figure(figsize=(10, 5))
    sns.barplot(data=venue_stats.head(5), y='Venue', x='Batter Runs', orient='h')
    sns.despine(bottom=True, trim=True)
    st.pyplot(plt)

    runs_by_innings = yuvraj_t20i.groupby(by='Innings').agg({'Batter Runs': 'sum', 'Batter Balls Faced': 'count'})
    ...
    explode = [0.1, 0]
    st.title("Yuvraj's Run Distribution Innings Wise")
    plt.figure(figsize=(8, 4))
    plt.pie(labels=labels, x=x, autopct='%1.1f%%', explode=explode)
    st.pyplot(plt)

    over_runs = yuvraj_t20i.groupby(by='Over').agg({"Batter Runs": 'sum'}).fillna(0).reset_index()

    st.title("Yuvraj's Top 5 Overs Where He Scored Maximum Runs")
    plt.figure(figsize=(8, 4))
    over_runs = over_runs.sort_values(by='Batter Runs', ascending=False)
    sns.barplot(data=over_runs.head(5), x='Over', y='Batter Runs')
    st.pyplot(plt)

    runs_bowlers = yuvraj_t20i.groupby(by='Bowler').agg({
        'Batter Runs': 'sum',
        'Match ID': 'count'
    }).reset_index()
    ...
    st.title("Yuvraj's Most Runs Against Different Bowlers")
    plt.figure(figsize=(8, 4))
    sns.barplot(data=runs_bowlers.head(5), y='Bowler', x='Batter Runs', orient='h', palette='viridis')
    st.pyplot(plt)

    total_fours = len(yuvraj_t20i[yuvraj_t20i['Batter Runs'] == 4])
    four_percentage = np.round(100 * total_fours * 4 / yuvraj_t20i['Batter Runs'].sum(), 2)
    total_sixes = len(yuvraj_t20i[yuvraj_t20i['Batter Runs'] == 6])
    six_percentage = np.round(100 * total_sixes * 6 / yuvraj_t20i['Batter Runs'].sum(), 2)

    runs_dist = pd.DataFrame(
        yuvraj_t20i['Batter Runs'].value_counts().reset_index().sort_values(by='index')
    )
    ...
    st.title("Yuvraj's Total Run Distribution with Singles, Doubles, Triples, Fours, and Sixes")
    plt.figure(figsize=(8, 4))
    labels = ['Dots', 'Singles', 'Doubles', 'Triples', 'Fours', 'Sixes']
    plt.pie(x=runs_dist['Runs'], labels=labels, autopct='%.2f', pctdistance=0.5)
    st.pyplot(plt)

    dismissal_type = pd.DataFrame(yuvraj_t20i['Method'].value_counts()).reset_index()
    dismissal_type.columns = ['Method', 'count']

    st.title("Yuvraj's Dismissals Throughout His Career")
    ...
    for index, row in yuvraj_t20i.iterrows():
    ...
    st.title("India Wins with Respect to Yuvraj's Scores")
    plt.figure(figsize=(8, 4))
    ...
    runs_by_opponent = yuvraj_t20i.groupby(by='Bat Second').agg({'Batter Runs': 'sum'}).reset_index()
    runs_by_opponent = runs_by_opponent[runs_by_opponent['Bat Second'] != 'India'].sort_values('Batter Runs', ascending=False)

    st.title("Yuvraj's Highest Scores Against Different Teams by Batting Second")
    plt.figure(figsize=(8, 4))
    ...
    runs_by_opponent = yuvraj_t20i.groupby(by='Bat First').agg({'Batter Runs': 'sum'}).reset_index()
    runs_by_opponent = runs_by_opponent[runs_by_opponent['Bat First'] != 'India'].sort_values('Batter Runs', ascending=False)

    st.title("Yuvraj's Highest Scores Against Different Teams by Batting First")
    plt.figure(figsize=(8, 4))
